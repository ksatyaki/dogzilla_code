...
class laserWarning(Node):
    ...
    def registerScan(self, scan_data):
        ...
        for i in range(len(ranges)):
            ...
            if 0.0 < ranges[i] < self.ResponseDist:
                ange_list.append(angle)
                dis_list.append(ranges[i])
            if angle > 180: angle = angle - 360
            if abs(angle) < self.LaserAngle and ranges[i] > 0:
                minDistList.append(ranges[i])
                minDistIDList.append(angle)
        
        if len(minDistIDList) > 10:
        ...
        if minDist <= self.ResponseDist: 
            ...
            self.Buzzer_state = False
            velocity1 = Twist()
            velocity1.angular.z = 0.0
            self.pub_vel.publish(velocity1)
